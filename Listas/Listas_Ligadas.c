...
#include <stdlib.h>
 ...
 struct snodo{ //snodo es el nombre de la estructura
    int valor;
    struct snodo *sig; //El puntero siguiente para recorrer  lista enlazada
}; 


void eliminar(struct snodo **cabeza, int dato){
...
int mostrar( struct snodo *cabeza){
	printf("\n ");
...
void ingresar(struct snodo **cabeza, int dato){
...
void ingresar_final(struct snodo** cabeza , int dato){
	struct snodo *nuevo;
	struct snodo *final;
	nuevo = NULL;
	final = NULL;
...
int main(void){
	...
	switch(op){
		...
		case 1 : printf("\nDame el dato: "); scanf("%d", &dato); ingresar(&cabeza, dato); break;
		case 2: mostrar(cabeza);  break;
		case 3:  printf("Dame el dato: "); scanf("%d", &dato); ingresar_final(&cabeza, dato); break;
		case 4: printf("Dime el dato que deseas eliminar: "); scanf("%d", &dato); eliminar(&cabeza, dato); break; 
